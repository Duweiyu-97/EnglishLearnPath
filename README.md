...
- **写作**：自定义题目、计时、字数统计、本地历史与可选 AI 反馈；题目区支持选择图片或 Ctrl+V 粘贴图表、地图、流程图和截图，并随作文永久保存在本地。使用 DeepSeek 时，带题图的批改会自动调用官方 `deepseek-v4-flash-vision-exp`，将图片与正文放进同一次多模态请求；纯文字题仍使用已配置的文字模型。不内置私人模板或课程讲义。
