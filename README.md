...
- **口语**：完整包内置 whisper.cpp 和 Whisper small.en，录音结束后在本机离线转写，音频不上传、无需 API Key。也可自行切换浏览器实时转写（可能联网）。可回听、校对并一起保存录音和文字稿，再按需调用文字 AI 批改。
...
- **原文标注与报告**：点击已批改记录进入报告页，原文中标红 AI 指出的精确片段，点击即可定位对应修改与原因；无法精确定位的建议保留在列表中。完整报告按标题分区并提供章节定位，标题不遮挡正文。可返回练习或编辑本题。记录以具体主题命名，题型作为辅助信息；查看旧报告不必再次请求 AI。
...
- **学习计划**：填写预计考试日期、当前与目标水平、重点和每日时间；手动计划覆盖从创建当天到考试日，AI 计划会按剩余天数拆分基础、强化、冲刺和考前调整阶段。首页读取当天所在阶段并直接进入对应练习。
...
口语只需录音一次：默认在结束后由内置 Whisper 离线转写，也可选择浏览器实时转写。校对后点击“保存录音与文字稿”，音频与文字稿作为同一条记录写入本地数据文件，刷新或重新启动后可从历史记录恢复回听，也可另行下载音频。单条音频最大 16 MB；音频不会发送给配置的文字 AI 接口。本地转写可取消或重试，失败时录音仍保留；浏览器模式的可用性依赖浏览器、权限与其语音服务，可能联网。
